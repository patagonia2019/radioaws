...
class StreamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stream" do
    assert_difference('Stream.count') do
      post streams_url, params: { stream: { name: @stream.name, station_id: @stream.station_id, type: @stream.type } }
    end
  ...
  test "should update stream" do
    patch stream_url(@stream), params: { stream: { name: @stream.name, station_id: @stream.station_id, type: @stream.type } }
